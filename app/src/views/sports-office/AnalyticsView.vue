...
async function loadPending() {
  try {
    pendingCount.value = await countPendingForRole("sports_office", auth.userId ?? "");
  } catch (e) {
  ...
  <div class="dash-analytics space-y-4">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500">Sports Office · Venue analytics</p>
      </div>
    ...
    <OfficeResourceAnalyticsPanel office="sports_office" :show-venues="true" :show-equipment="false" />
  </div>
</template>
