...
import OfficerCreateEventModal from "./components/OfficerCreateEventModal.vue";
import OfficerEventDetailModal from "./components/OfficerEventDetailModal.vue";
...
const reminderStorageKey = computed(() => `eventlink:officer:approved-reminder-seen:${auth.userId ?? "anon"}`);
const showApprovedReminder = computed(
  () => approvedNoticeCount.value > 0 && reminderToken.value !== seenReminderToken.value,
);

const calendarEvents = computed(() =>
  mapPortalEventsToCalendar(
    scheduledEvents.value.map((e) => ({ ...e, organization: e.organization ?? e.department })),
  ),
);
...
function workflowRowClass(event: OfficerEvent) {
...
<template>
  <div class="dash-page">
    ...
    <OfficerCreateEventModal :open="createOpen" @close="createOpen = false" />
    <OfficerEventDetailModal
      :event="selectedEvent"
