...
import AdviserApprovedEventModal from "./components/AdviserApprovedEventModal.vue";
import EventLetterDownloadButton from "@/components/portal/EventLetterDownloadButton.vue";

const { approvedEvents } = useAdviserPortal();
const search = ref("");
...
function openRow(event: AdviserEvent) {
...
<template>
  <div class="dash-page flex flex-col">
    <div class="mb-6 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
      <div>
        ...
        <p class="text-gray-500 text-sm mt-1">
          Approved events for CCSICT. <strong>Download Word</strong> is the file the organization uploaded with the
          request.
        </p>
      </div>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right" @click.stop>
                <EventLetterDownloadButton :event="event" size="sm" />
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <AdviserApprovedEventModal v-if="selectedEvent" :event="selectedEvent" @close="selectedEvent = null" />
  </div>
</template>
