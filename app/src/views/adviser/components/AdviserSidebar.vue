...
import { LayoutDashboard, BarChart2, X, ChevronRight, UserCircle, Radar } from "lucide-vue-next";
...
const navItems = [
  { label: "Dashboard", to: "/adviser/analytics", name: "adviser-analytics", icon: BarChart2 },
  { label: "Event Management", to: "/adviser", name: "adviser-dashboard", icon: LayoutDashboard },
  { label: "Event Monitoring", to: "/adviser/monitoring", name: "adviser-monitoring", icon: Radar },
  { label: "Profile", to: "/adviser/profile", name: "adviser-profile", icon: UserCircle },
] as const;
...
<template>
  <div class="portal-sidebar-slot">
    ...
    >
      <div class="flex items-center justify-between border-b border-white/10 px-4 py-3 sm:px-5 sm:py-3.5">
        <div class="flex min-w-0 items-center gap-2.5">
          ...
          <div>
            ...
            <div class="text-[#4ADE80] text-[10px] font-semibold uppercase tracking-wider">Adviser Portal</div>
          </div>
        </div>
        <button
          type="button"
      ...
      <PortalSidebarFooter portal-label="Adviser" />
    </aside>
  </div>
</template>
