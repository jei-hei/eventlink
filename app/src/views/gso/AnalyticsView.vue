...
async function loadPending() {
  try {
    pendingCount.value = await countPendingForRole("gso", auth.userId ?? "");
  } catch (e) {
  ...
  <div class="dash-analytics space-y-4">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500">GSO · Venue and resource analytics</p>
      </div>
    ...
    <OfficeResourceAnalyticsPanel office="gso" :show-venues="true" :show-equipment="true" />
  </div>
</template>
