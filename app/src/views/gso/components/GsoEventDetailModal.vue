...
function statusClass(status: GsoEvent["status"]) {
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden max-h-[90vh] flex flex-col">
      <div class="bg-[#16A34A] text-white px-6 py-4 flex items-center justify-between shrink-0">
        <h3 class="font-bold text-base">
          {{ event.createdBy === "EO" ? "GSO Event Request" : "Venue/Equipment Request" }}
        </h3>
      ...
      <div class="p-6 space-y-4 overflow-auto flex-1 min-h-0">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Number of Participants</label>
            <p class="text-gray-800 font-medium">{{ event.numberOfParticipants ?? "N/A" }}</p>
          </div>
          <div>
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">SDG/s</label>
            <p class="text-gray-800 font-medium">{{ event.sdgs ?? "N/A" }}</p>
          </div>
        </div>
