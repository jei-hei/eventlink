...
import { LayoutDashboard, ClipboardList, BarChart2, X, ChevronRight, UserCircle, MapPin, Package } from "lucide-vue-next";
...
const navItems = [
  ...
  { label: "Equipment", to: "/ssc/equipment", name: "ssc-equipment", icon: Package },
    ...
    >
      <div class="flex items-center justify-between border-b border-white/10 px-4 py-3 sm:px-5 sm:py-3.5">
        <div class="flex min-w-0 items-center gap-2.5">
          <div
            class="flex h-8 min-w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 px-1 shadow-md ring-1 ring-white/20"
          >
            <span class="text-[10px] font-bold leading-none text-white">SSC</span>
          </div>
          <div>
            <div class="text-white text-sm font-bold leading-tight">EventLink</div>
            <div class="text-[#4ADE80] text-[10px] font-semibold uppercase tracking-wider">
              SSC Portal
            </div>
          </div>
        </div>
      ...
      <PortalSidebarFooter portal-label="SSC" />
    </aside>
  </div>
</template>
